...
function FunnelPanel({ funnel }) {
  ...
  return (
    <Panel title="Campaign Progress Overview">
      <div className="funnel" style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
        {data.map((d, i) => (
          <div
            key={d.name}
            style={{
          ...
          >
            <span>{d.name}</span>
            <span style={{ marginLeft: '12px' }}>{d.v}</span>
          </div>
        ))}
      </div>
...
export default function ManagerDashboard() {
  ...
  return (
    <Layout role="manager" active="Dashboard">
      ...
      <div className="two" style={{ display: 'flex', gap: '24px', flexWrap: 'wrap' }}>
        <div style={{ flex: 1, minWidth: '300px', overflow: 'hidden' }}>
          <FunnelPanel funnel={dashboard.funnel} />
        </div>
        
        <div style={{ flex: 1, minWidth: '300px', overflow: 'hidden' }}>
          <Panel title="MR-wise Performance" action="View All" onActionClick={handleViewAllMRs}>
            <div style={{ overflowX: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: '500px' }}>
                <thead>
                  <tr style={{ background: '#f9fafb', borderBottom: '1px solid #e5e7eb' }}>
                    <th style={{ padding: '12px', textAlign: 'left' }}>MR Name</th>
                    <th style={{ padding: '12px', textAlign: 'left' }}>Total Doctors</th>
                    <th style={{ padding: '12px', textAlign: 'left' }}>Approved (%)</th>
                    <th style={{ padding: '12px', textAlign: 'left' }}>Input Given (%)</th>
                ...
                <tbody>
                  {dashboard.mrPerformance?.map((mr, idx) => (
                    <tr key={idx} style={{ borderBottom: '1px solid #f3f4f6' }}>
                      ...
                      <td style={{ padding: '12px' }}>{mr.totalDoctors || 0}</td>
                      <td style={{ padding: '12px' }}>{mr.approvedDoctors || 0} ({mr.approvedPercentage || 0}%)</td>
                      <td style={{ padding: '12px' }}>{mr.inputGivenDoctors || 0} ({mr.inputGivenPercentage || 0}%)</td>
                      <td style={{ padding: '12px' }}><Badge tone="red">{mr.pendingDoctors || 0}</Badge></td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      <div className="two" style={{ display: 'flex', gap: '24px', flexWrap: 'wrap', marginTop: '24px' }}>
        <div style={{ flex: 1, minWidth: '300px', overflow: 'hidden' }}>
        ...
        <div style={{ flex: 1, minWidth: '300px', overflow: 'hidden' }}>
          <Panel title="Recent Activity" action="View All" onActionClick={handleViewAllActivities}>
              ...
              >
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start' }}>
                  <div>
                    ...
                    <p style={{ margin: 0, fontSize: '12px', color: '#666' }}>{doctor.speciality}</p>
                    <p style={{ margin: '8px 0 0 0', fontSize: '12px', color: '#999' }}>
                      Added: {new Date(doctor.createdAt).toLocaleDateString()}
                    </p>
                  </div>
                  <Badge tone={doctor.approvalStatus === "approved" ? "green" : "orange"}>
